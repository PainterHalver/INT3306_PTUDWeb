...
export default function FaultyProductsFromBaohanh() {
  ...
  const receiveFaultyProductsFromBaohanh = async (product_ids: number[]) => {
    try {
      ...
      const res = await axios.post<any, AxiosResponse<any, any>, ReceivePayload>("/products/receive", {
        status: "loi_da_dua_ve_co_so_san_xuat",
        product_ids,
      });
  ...
  const confirmReceive = async () => {
    try {
      await receiveFaultyProductsFromBaohanh(selectedIds);
